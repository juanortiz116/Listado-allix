...
import { Settings, Calculator, Book } from 'lucide-react';
import { clsx } from 'clsx';
import { useStore } from './store/useStore';

function App() {
  const [view, setView] = useState<'configurator' | 'budget' | 'catalog'>('budget');
    ...
    <div className="h-screen flex flex-col">
      ...
      <nav className="bg-surface border-b border-border p-2 flex justify-center gap-4">
        ...
        <button
          ...
          className={clsx(
            "flex items-center gap-2 px-6 py-2 rounded-lg font-bold transition-all",
            view === 'catalog' ? "bg-accent text-emerald-950 shadow-lg shadow-emerald-900/20" : "text-gray-400 hover:text-white"
          )}
        >
